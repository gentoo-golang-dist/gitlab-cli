...
// DirectEditor directly opens an external editor with the given content, bypassing
// the huh interactive text field. This is useful when the user explicitly requests
// to use an external editor (e.g., via -d- flag).
func (s *IOStreams) DirectEditor(ctx context.Context, result *string, defaultContent, editorCmd string) error {
	...
	if len(editorParts) > 1 {
		...
		cmd = exec.CommandContext(ctx, editorParts[0], tmpPath)
	}
