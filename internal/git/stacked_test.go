...
func Test_StackRootDir(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Contains(t, got, stackedDirName, "StackRootDir() should contain stackedDirName")
...
func Test_GetStacks(t *testing.T) {
	tests := []struct {
		...
		{name: "normal repo"},
		{name: "worktree", worktree: true},
	}

	for _, tt := range tests {
		t.Run(tt.name+"/two stacks", func(t *testing.T) {
			stacks := []Stack{
				{
					Title: "stack-0",
		...
		t.Run(tt.name+"/no stacks", func(t *testing.T) {
			InitGitRepoOrWorktree(t, tt.worktree)
			got, err := GetStacks()
			var want []Stack = nil
