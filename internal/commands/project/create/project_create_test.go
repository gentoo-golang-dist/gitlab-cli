...
func Test_projectCreateCmd(t *testing.T) {
	...
	origGitInitFunc := gitInitFunc
	origInitialiseRepoFunc := initialiseRepoFunc

	defer func() {
		createProject = origCreateProject
		currentUser = origCurrentUser
		addRemote = origAddRemote
		gitInitFunc = origGitInitFunc
		initialiseRepoFunc = origInitialiseRepoFunc
	}()
	...
	testCases := []struct {
	...
	}{
		{
			...
			SetupMocks: func() {
				...
				gitInitFunc = func(defaultBranch string) error {
					return nil
				}
				initialiseRepoFunc = func(projectPath, remoteURL string) error {
					return nil
				}
			},
		},
		...
		},
		{
			...
			SetupMocks: func() {
				...
				gitInitFunc = func(defaultBranch string) error {
					...
					t.Error("gitInitFunc should not be called when already in a git repository")
					return nil
				}
			},
			wantErr: false,
