...
func NewCmdList(f cmdutils.Factory) *cobra.Command {
	...
	repoListCmd.Flags().IntVarP(&opts.perPage, "per-page", "P", 30, "Number of items to list per page. Maximum: 100.")
	cmdutils.EnableJSONOutput(repoListCmd, opts.io, &opts.outputFormat)
	repoListCmd.Flags().BoolVarP(&opts.filterAll, "all", "a", false, "List all projects on the instance (removes ownership filter; results are still paginated — use --page to navigate).")
