...
func NewCmdRemoteAdd(f cmdutils.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Doc(`
			...
			The remote name defaults to the first path component (the namespace or owner),
			following the same convention as "hub remote add".
		`),
		Example: heredoc.Doc(`
			# Add a remote for a collaborator's fork (remote named "alice")
			$ glab repo remote add alice/my-project

			# Add a remote with a custom name
			$ glab repo remote add alice/my-project --name upstream

			# Add a remote for a project in a subgroup (remote named "group")
