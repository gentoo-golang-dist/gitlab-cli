...
func factoryOpts(t *testing.T, tc *gitlabtesting.TestClient, remotes glrepo.Remotes) []cmdtest.FactoryOption {
	t.Helper()
...
func TestNewCmdRemoteAdd(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			stubShellouts:     []string{"git remote add -f alice git@gitlab.com:alice/my-project.git"},
		...
		},
		{
			...
			stubShellouts:     []string{"git remote add -f upstream git@gitlab.com:alice/my-project.git"},
		...
		},
		{
			...
			stubShellouts:     []string{"git remote add -f group git@gitlab.com:group/subgroup/project.git"},
		...
		},
		{
			...
			stubShellouts:     []string{"git remote add -f alice https://gitlab.com/alice/my-project.git"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, stub := range tt.stubShellouts {
				cs.Stub(stub)
			}
			...
			opts := factoryOpts(t, tc, tt.remotes)
			exec := cmdtest.SetupCmdForTest(t, NewCmdRemoteAdd, false, opts...)
