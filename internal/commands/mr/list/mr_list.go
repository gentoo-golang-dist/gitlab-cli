...
func NewCmdList(f cmdutils.Factory, runE func(opts *options) error) *cobra.Command {
	...
	mrListCmd.Flags().BoolVarP(&opts.all, "all", "A", false, "Get all merge requests. (default false)")
	mrListCmd.Flags().BoolVarP(&opts.closed, "closed", "c", false, "Get only closed merge requests. (default false)")
	mrListCmd.Flags().BoolVarP(&opts.merged, "merged", "M", false, "Get only merged merge requests. (default false)")
	mrListCmd.Flags().BoolVarP(&opts.draft, "draft", "d", false, "Filter by draft merge requests. (default false)")
	mrListCmd.Flags().BoolVarP(&opts.notDraft, "not-draft", "", false, "Filter by non-draft merge requests. (default false)")
