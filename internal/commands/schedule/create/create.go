...
func NewCmdCreate(f cmdutils.Factory) *cobra.Command {
	scheduleCreateCmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
		Create a new CI/CD pipeline schedule. The %[1]s--cron%[1]s and %[1]s--ref%[1]s flags
		are typically required: %[1]s--cron%[1]s sets the schedule's recurrence in
		cron syntax, and %[1]s--ref%[1]s sets the branch or tag the pipeline runs
		against.
