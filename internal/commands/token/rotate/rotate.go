...
func NewCmdRotate(f cmdutils.Factory) *cobra.Command {
	...
	cmd.Flags().VarP(&opts.duration, "duration", "D", "Sets the token lifetime in days. Accepts: days (30d), weeks (4w), or hours (720h). Maximum: 365d. The token expires at midnight UTC on the calculated date.")
...
func (o *options) complete(cmd *cobra.Command, args []string) error {
	...
	if time.Time(o.expireAt).IsZero() {
		// Calculate expiration date in UTC to avoid timezone ambiguity.
		// The API only accepts dates (YYYY-MM-DD), not times.
		// We add the duration to the current time in UTC, then set the time to midnight UTC.
		endTime := time.Now().UTC().Add(o.duration.Duration())
		endDate := time.Date(endTime.Year(), endTime.Month(), endTime.Day(), 0, 0, 0, 0, time.UTC)
		o.expireAt = expirationdate.ExpirationDate(endDate)
	}
...
func (o *options) validate() error {
	...
	if duration%(24*time.Hour) != 0 {
		return cmdutils.FlagError{Err: errors.New("duration must be in whole days")}
	}

	// Validate duration is between 1 and 365 days
	if duration < 24*time.Hour || duration > 365*24*time.Hour {
		return cmdutils.FlagError{Err: errors.New("duration must be between 1 and 365 days")}
	}
