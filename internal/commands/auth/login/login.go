...
func loginRun(ctx context.Context, opts *LoginOptions) error {
	...
	if hostname == "" {
		...
		if detectErr == nil && len(detectedHosts) > 0 {
			...
			isSelfHosted = selectedOption == "GitLab Self-Managed or GitLab Dedicated instance"
	...
	if opts.Interactive {
		loginTypeOptions := []string{"Token", "Web"}
	...
	if opts.Interactive {
		gitProtocolOptions := []string{"SSH", "HTTPS", "HTTP"}
		gitProtocol = "HTTPS" // Set default
		...
		if isSelfHosted {
			apiProtocolOptions := []string{"HTTPS", "HTTP"}
			apiProtocol = "HTTPS" // Set default
