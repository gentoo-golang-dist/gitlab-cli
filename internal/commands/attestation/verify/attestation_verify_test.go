...
func Test_AttestationVerify(t *testing.T) {
	...
	keyResponse := `
  {
	...
	apiEndpoint := fmt.Sprintf("/api/v4/projects/%s/attestation/%s", repoName, subjectDigest)
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			output, err := runCommand(t, fakeHTTP, subjectDigest)
