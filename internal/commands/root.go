...
func NewCmdRoot(f cmdutils.Factory) *cobra.Command {
	...
	rootCmd := &cobra.Command{
		...
		Annotations: map[string]string{
			...
			Can be set in the config with %[1]sglab config set browser mybrowser%[1]s.
			...
			GITLAB_CLIENT_ID: Provide custom %[1]sclient_id%[1]s generated by GitLab OAuth 2.0 application.
			Defaults to the %[1]sclient-id%[1]s for GitLab.com.
			...
			Can be set in the config with %[1]sglab config set token xxxxxx%[1]s.
			...
			Can be set in the config with %[1]sglab config set telemetry false%[1]s.
			...
			REMOTE_ALIAS or GIT_REMOTE_URL_VAR: A %[1]sgit remote%[1]s variable or alias that contains
			the GitLab URL. Can be set in the config with %[1]sglab config set remote_alias origin%[1]s.

			VISUAL, EDITOR (in order of precedence): The editor tool to use for authoring text.
			Can be set in the config with %[1]sglab config set editor vim%[1]s.
