...
func renderText(streams *iostreams.IOStreams, wi *workitemsapi.WorkItem) error {
	...
	stateTint := utils.StateColor(wi.State, c)
	header := fmt.Sprintf("%s #%s · %s · %s", wi.WorkItemType.Name, wi.IID, stateTint(wi.State), wi.WebURL)
	...
	if wi.Children != nil && wi.Children.Count > 0 {
		...
		for _, child := range wi.Children.Nodes {
			tint := utils.StateColor(child.State, c)
