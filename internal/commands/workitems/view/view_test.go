...
// respondWithGroupNode is the group-scoped twin of
// respondWithProjectNode, used when the test passes -g and we
// expect the GraphQL response under data.group.
func respondWithGroupNode(wi *workitemsapi.WorkItem, assertVars func(gitlab.GraphQLQuery)) func(gitlab.GraphQLQuery, any, ...gitlab.RequestOptionFunc) (*gitlab.Response, error) {
	return func(query gitlab.GraphQLQuery, response any, _ ...gitlab.RequestOptionFunc) (*gitlab.Response, error) {
		if assertVars != nil {
			assertVars(query)
		}
...
func TestWorkItemsView_GroupScope(t *testing.T) {
	out, err := runView(t, "7 -g gitlab-org", func(tc *gitlabtesting.TestClient) {
		tc.MockGraphQL.EXPECT().
			Do(gomock.Any(), gomock.Any(), gomock.Any()).
			DoAndReturn(respondWithGroupNode(sampleWorkItem(), func(q gitlab.GraphQLQuery) {
				assert.Equal(t, "gitlab-org", q.Variables["groupPath"])
				assert.Equal(t, "7", q.Variables["iid"])
				_, hasProjectPath := q.Variables["projectPath"]
				assert.False(t, hasProjectPath, "group scope must not set projectPath")
			}))
	})
	require.NoError(t, err)
	assert.Contains(t, out, "Issue #42")
	assert.Contains(t, out, "Plan Q2 roadmap")
}
