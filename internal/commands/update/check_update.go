...
func PrintUpdateError(streams *iostreams.IOStreams, err error, cmd *cobra.Command, debug bool) {
	...
	if errors.As(err, &dnsError) {
		...
		streams.LogErrorf("%s Check your internet connection and status.gitlab.com. If on GitLab Self-Managed, run 'sudo gitlab-ctl status' on your server.\n", color.DotWarnIcon())
	} else {
		...
		if errors.As(err, &exitError) {
			...
			if errors.As(err, &flagError) || strings.HasPrefix(err.Error(), "unknown command ") {
				if cmd != nil {
					streams.LogErrorf("Try '%s --help' for more information.", cmd.CommandPath())
				} else {
					streams.LogError("Try --help for more information.")
				}
			}
		}
	}
