...
// HuhTheme returns a huh ThemeFunc with GitLab product colors.
...
func HuhTheme() huh.ThemeFunc {
	return func(isDark bool) *huh.Styles {
		...
		if isDark {
			gitlabPurple = lipgloss.Color("#A989F5")
			gitlabOrange = lipgloss.Color("#FC6D26")
			gitlabBlue = lipgloss.Color("#4285F4")
			gitlabRed = lipgloss.Color("#F97583")
			gitlabSubtle = lipgloss.Color("#B0B0B0")
		} else {
		...
		t.Focused.Base = t.Focused.Base.BorderForeground(gitlabPurple)
		t.Focused.Title = t.Focused.Title.Foreground(gitlabPurple).Bold(true)
		t.Focused.Description = t.Focused.Description.Foreground(gitlabSubtle)
		t.Focused.SelectedOption = t.Focused.SelectedOption.Foreground(gitlabBlue)
		t.Focused.UnselectedOption = t.Focused.UnselectedOption.Foreground(gitlabSubtle)
		t.Focused.SelectedPrefix = lipgloss.NewStyle().Foreground(gitlabOrange).SetString("✓ ")
		t.Focused.UnselectedPrefix = lipgloss.NewStyle().Foreground(gitlabSubtle).SetString("• ")
		t.Focused.MultiSelectSelector = t.Focused.MultiSelectSelector.Foreground(gitlabOrange)
		t.Focused.SelectSelector = t.Focused.SelectSelector.Foreground(gitlabOrange)

		t.Focused.TextInput.Cursor = t.Focused.TextInput.Cursor.Foreground(gitlabOrange)
		t.Focused.TextInput.Placeholder = t.Focused.TextInput.Placeholder.Foreground(gitlabSubtle)
		t.Focused.TextInput.Prompt = t.Focused.TextInput.Prompt.Foreground(gitlabPurple)

		t.Blurred.Base = t.Blurred.Base.BorderForeground(gitlabSubtle)
		t.Blurred.Title = t.Blurred.Title.Foreground(gitlabSubtle)
		t.Blurred.Description = t.Blurred.Description.Foreground(gitlabSubtle)
		t.Blurred.TextInput.Prompt = t.Blurred.TextInput.Prompt.Foreground(gitlabSubtle)
		...
		if isDark {
			subtleBackground = lipgloss.Color("#3a3a3a")
		} else {
			subtleBackground = lipgloss.Color("#e0e0e0")
		}
		...
		t.Focused.ErrorIndicator = t.Focused.ErrorIndicator.Foreground(gitlabRed)
		t.Focused.ErrorMessage = t.Focused.ErrorMessage.Foreground(gitlabRed)
		t.Blurred.ErrorIndicator = t.Blurred.ErrorIndicator.Foreground(gitlabRed)
		t.Blurred.ErrorMessage = t.Blurred.ErrorMessage.Foreground(gitlabRed)
