...
if [ -z "$os" ] || [ -z "$artifact" ]; then
	echo "Usage: $0 <macos|windows> <artifact_path>"
	exit 1
fi
...
if [ ! -f "$artifact" ]; then
	echo "ERROR: Artifact file does not exist: $artifact"
	exit 1
fi
...
if [ -z "$GCLOUD_PROJECT" ] || [ -z "$GOOGLE_APPLICATION_CREDENTIALS" ] || [ -z "$HOST_PWD" ]; then
	...
	echo "Required: GCLOUD_PROJECT, GOOGLE_APPLICATION_CREDENTIALS, HOST_PWD"
	exit 1
fi
