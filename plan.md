...
- `GetLatestDiffVersion(client, project, mrIID)` — fetch latest MR diff version with SHAs
- `BuildDiffPosition(version, filePath, lineStart, lineEnd, oldLine)` → `*gitlab.PositionOptions`
- `lineCode(path, line)` helper
- Discussion ID prefix resolution: `ResolveDiscussionID(client, project, mrIID, prefix)` → full ID
