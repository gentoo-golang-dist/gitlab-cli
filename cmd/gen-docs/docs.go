...
// detectAndWrapURLs detects bare URLs in text and wraps them in markdown link syntax.
...
func detectAndWrapURLs(text string) string {
	...
	// The pattern captures URLs that continue until whitespace, closing parenthesis, or pipe character
	urlPattern := regexp.MustCompile(`(https?://[^\s\)\|]+)`)

	return urlPattern.ReplaceAllStringFunc(text, func(url string) string {
		// Find the position of this URL in the original text
		...
		// by checking if it's preceded by '](' in the original text
		if idx >= 2 && text[idx-2:idx] == "](" {
			// Already wrapped, return as-is
			return url
		}

		// Check if this URL is inside backticks
		// Count backticks before this URL
		backticksBefore := strings.Count(text[:idx], "`")
		// If odd number of backticks before, we're inside a backtick block
		if backticksBefore%2 == 1 {
			// Inside backticks, don't wrap (these are typically literal values)
			return url
		}
...
func GenMarkdownCustom(cmd *cobra.Command, w io.Writer) error {
	...
	shortDesc := detectAndWrapURLs(cmd.Short)
	...
	if len(cmd.Long) > 0 {
		...
		if cmd.Name() != "help" {
			...
			longDesc := detectAndWrapURLs(cmd.Long)
			...
			if len(lines) > 0 {
				...
				if strings.Contains(firstLine, ":") {
					...
					if len(parts) == 2 {
						...
						description = detectAndWrapURLs(description)
