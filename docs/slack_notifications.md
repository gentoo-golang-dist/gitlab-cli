...
The `.gitlab-ci.yml` release job needs to pass the Slack webhook environment variable to the GoReleaser Docker container.

Update the `release` job in `.gitlab-ci.yml`:
...
To test the Slack notification without creating a real release:
