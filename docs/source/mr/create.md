...
```plaintext
  ...
  -a, --assignee usernames     Assign merge request to people by their usernames. Repeat flag for multiple assignees.
  ...
  -i, --related-issue string   Create a merge request for an issue. If --title is not provided, uses the issue title.
      ...
      --reviewer usernames     Request review from users by their usernames. Repeat flag for multiple reviewers.
