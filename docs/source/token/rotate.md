...
```plaintext
  -D, --duration duration   Sets the token lifetime in days. Accepts: days (30d), weeks (4w), or hours (720h). Maximum: 365d. The token expires at midnight UTC on the calculated date. (default 30d)
